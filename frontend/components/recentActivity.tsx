...
export function RecentActivityCard({ isLoading, transactions, tokenSymbol }: RecentActivityCardProps) {
          ...
          <div className="space-y-4">
            {transactions.map((activity: any) => (
              <div key={activity.id} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`h-10 w-10 rounded-full flex items-center justify-center ${
                      activity.type === "send" ? "bg-green-100" : "bg-orange-100"
                    }`}
                  >
                    {activity.type === "send" ? (
                ...
                <div className={`font-medium ${activity.type === "send" ? "text-green-600" : "text-orange-600"}`}>
                  {activity.type === "send" ? "+" : "-"}
